...
kernels — ``add_spatial_constraint`` and ``central_sulcus`` — run as
batched pull-based Bellman-Ford on device (one (V, K) distance matrix
per problem family, BF iterations until convergence). The remaining
small CPU pieces — parcel CSR build, paracentral/insula classifier,
truncate decision table — stay CPU: heavy control-flow that runs in
~1 ms each, not worth a kernel.
...
Hybrid layout per hemi:
    ...
    GPU:       central_sulcus (Bellman-Ford to full settle), then
...
def _run_hemi_gpu(hemi: str, mesh_d: dict, labels: np.ndarray,
    ...
    # ── central_sulcus on GPU (batched BF, unbounded SSSP) ──
    # One source per column in the (V, Nsrc) distance matrix.
    Npre = int(pre_verts.size)
    Npost = int(post_verts.size)
    Nsrc = Npre + Npost
    src_concat = np.concatenate([pre_verts, post_verts]).astype(np.int64)
    src_concat_dev = cp.asarray(src_concat)

    t = time.perf_counter()
    dist_src_dev = cp.empty((V, Nsrc), dtype=cp.float32)
    scratch_src_dev = cp.empty_like(dist_src_dev)
    init_dist_for_sources_cupy(src_concat_dev, dist_src_dev)
    # Unbounded BF (radius = +inf disables the clamp).
    bellman_ford_bounded_cupy(
        indptr_dev, indices_dev, weights_dev,
        dist_src_dev, scratch_src_dev,
        radius=float("inf"), max_iters=500,
    )
    # Per-parcel mean: avg_dis[h, l] = mean over (s_h, v ∈ parcel l) of dist[v, s_h].
    # Sum distances over (parcel-verts × source-side) on device.
    # dist_src_dev is (V, Nsrc). Pre slice = cols [0, Npre); post slice = cols [Npre, Nsrc).
    avg_dis = np.empty((2, L_h), dtype=np.float64)
    # Reduce per source-side: (V, Npre) -> per-vert sum -> per-parcel mean.
    # Doing this entirely on device with one matmul: 1_pre @ dist[:, :Npre] = (V,) per-v sum_pre.
    sum_pre_per_v_dev = dist_src_dev[:, :Npre].astype(cp.float64).sum(axis=1)
    sum_post_per_v_dev = dist_src_dev[:, Npre:].astype(cp.float64).sum(axis=1)
    sum_pre_per_v = cp.asnumpy(sum_pre_per_v_dev)
    sum_post_per_v = cp.asnumpy(sum_post_per_v_dev)
    ...
    for l_idx in range(L_h):
        ...
        if n_v == 0:
            ...
            continue
        vs = parcel_inds[ps:pe]
        avg_dis[0, l_idx] = float(sum_pre_per_v[vs].sum()) / (float(n_v) * Npre_f)
        avg_dis[1, l_idx] = float(sum_post_per_v[vs].sum()) / (float(n_v) * Npost_f)
    # Done with the source distance matrix; release back to pool.
    del dist_src_dev, scratch_src_dev
